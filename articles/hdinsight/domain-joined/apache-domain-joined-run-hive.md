...
在[設定已加入網域的 HDInsight 叢集](apache-domain-joined-configure.md#create-and-configure-azure-ad-ds-for-your-azure-ad)中，您已建立 hiveruser1 和 hiveuser2。 在本教學課程中，您將使用兩個使用者帳戶。

## <a name="create-ranger-policies"></a>建立 Ranger 原則
在這一節中，您將建立兩個 Ranger 原則以供存取 hivesampletable。 您會提供不同資料行集的選取權限。 兩個使用者都建立於[設定已加入網域的 HDInsight 叢集](apache-domain-joined-configure.md#create-and-configure-azure-ad-ds-for-your-azure-ad)。  在下一節中，您將在 Excel 中測試這兩個原則。
