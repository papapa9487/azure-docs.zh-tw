...
下列程式碼範例會說明範例 **submitSubclipCallback** 方法。 在此方法中，您會實作 MES 編碼預設的 HTTP POST。 如果 POST 成功 (**結果**)，則**承諾**已解決，否則就會拒絕且具有錯誤詳細資料。
...
```
作業提交的輸出是轉譯作業的 MES 編碼預設，或是動態資訊清單篩選條件的 REST API 裝載。

## <a name="rendered-output"></a>轉譯的輸出
...
## <a name="filter-output"></a>篩選條件輸出
篩選條件剪輯的輸出合約是具有下列屬性的 JSON 物件：
