...
1. 新增套件。 填寫下列表單上的 [套件詳細資料] 區段：
   ...
   * **選取套件檔案**：此套件包含壓縮成 .zip 套件的兩個必要檔案。 其中一個檔案為資源管理員範本，此範本定義要與受管理應用程式一起部署的資源。 另一個檔案會針對透過入口網站部署受管理應用程式的取用者，定義[使用者介面](create-uidefinition-overview.md)。 在使用者介面中，您可以指定讓取用者提供參數值的項目。
