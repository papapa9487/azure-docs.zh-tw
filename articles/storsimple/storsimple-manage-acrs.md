...
description: "說明如何使用存取控制記錄 (ACR) 來判斷哪些主機可以連線到 StorSimple 裝置上的磁碟區。"
services: storsimple
documentationcenter: 
author: alkohli
manager: carmonm
...
> [!NOTE]
> StorSimple 的傳統入口網站已過時。 按照淘汰排程，StorSimple 裝置管理員會自動移至新的 Azure 入口網站。 您將收到關於此移動的電子郵件和入口網站通知。 本文件也很快就會淘汰。 若要檢視適用於新 Azure 入口網站的文章版本，請移至[使用 StorSimple Manager 服務管理存取控制記錄](storsimple-8000-manage-acrs.md)。 若有關於移動的任何問題，請參閱[常見問題集：移至 Azure 入口網站](storsimple-8000-move-azure-portal-faq.md)。

## <a name="overview"></a>概觀
存取控制記錄 (ACR) 可讓您指定哪些主機可連線到 StorSimple 裝置上的磁碟區。 ACR 設為特定的磁碟區，並且包含主機的 iSCSI 限定名稱 (IQN)。 當主機嘗試連線到磁碟區時，裝置會檢查與該磁碟區相關聯之 ACR 的 IQN 名稱，如果相符，則會建立連線。 [設定] 頁面上的存取控制記錄區段會顯示具有主機對應 IQN 的所有存取控制記錄。
...
> [!IMPORTANT]
> * 將 ACR 指派到磁碟區時，請注意磁碟區並未被多個非叢集主機並行存取，因為這可能會造成磁碟區損毀。 
...
1. 在服務登陸頁面上，選取您的服務，按兩下該服務名稱，然後按一下 [設定] 索引標籤。
...
3. 刪除圖示 (**x**) 會出現在您選取之 ACR 資料行的最右邊。 按一下 [x]**x** 圖示以刪除 ACR。
4. 當系統提示您確認時，請按一下 [是] 繼續進行刪除。 表格式清單會更新以反映刪除。
