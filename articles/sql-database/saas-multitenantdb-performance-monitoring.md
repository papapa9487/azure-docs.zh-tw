...
[Azure 入口網站](https://portal.azure.com)提供大部分資源的內建監視與警示功能。 針對 SQL Database，可使用資料庫監視與警示功能。 這個內建的監視與警示功能是資源特定，因此針對少數資源使用很方便，但是搭配許多資源使用時則不是很方便。
...
> [!IMPORTANT]
> 負載產生器正於您新的 PowerShell 視窗中以一系列作業的方式執行。 如果您關閉工作階段，負載產生器就會停止。 負載產生器會維持 job-invoking 狀態，它會在產生器啟動後佈建的任何新租用戶上產生負載。 使用 *CTRL-C* 來停止叫用新作業並結束指令碼。 負載產生器將會繼續執行，但只在現有租用戶上執行。
