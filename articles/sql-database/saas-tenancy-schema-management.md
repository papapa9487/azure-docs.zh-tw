...
[第一個 Wingtip Tickets SaaS 每一租用戶一個資料庫教學課程](saas-dbpertenant-get-started-deploy.md)說明應用程式如何佈建租用戶資料庫並在目錄中註冊它。 就像任何應用程式一樣，Wingtip Tickets SaaS 每一租用戶一個資料庫應用程式會隨著時間演進，而且有時候會需要變更資料庫。 變更可能包含新增或變更的結構描述、新增或變更的參考資料，以及例行性資料庫維護工作以確保最佳的應用程式效能。 有了 SaaS 應用程式，這些變更需要以經過協調的方式部署到可能非常大規模的租用戶資料庫群。 變更也需要整合到佈建程序中，以供未來在租用戶資料庫中使用。
...
![畫面](media/saas-tenancy-schema-management/schema-management.png)
...
1. 開啟 SSMS 並連線到類別目錄伺服器：catalog-dpt-\<user\>.database.windows.net server
1. 另外也連線到租用戶伺服器：tenants1-dpt-\<user\>.database.windows.net
1. 瀏覽至 *tenants1-dpt-\<user\>* 伺服器上的 *contosoconcerthall* 資料庫，並查詢 *VenueTypes* 資料表以確認 *Motorcycle Racing* (機車賽) 和 *Swimming Club* (游泳俱樂部) **不存在於**結果清單中。
