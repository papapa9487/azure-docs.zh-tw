...
在本教學課程中，您會在整個租用戶資料庫集合執行分散式查詢以啟用臨機操作互動式報表。 這些查詢可以擷取藏在 Wingtip Tickets SaaS 應用程式日常操作資料中的深入解析。 若要這麼做，您要將額外的分析資料庫部署到目錄伺服器，並使用「彈性查詢」來啟用分散式查詢。
...
在 [WingtipTicketsSaaS-DbPerTenant](https://github.com/Microsoft/WingtipTicketsSaaS-DbPerTenant/) github 存放庫中可取得 Tickets SaaS Database Per Tenant 指令碼和應用程式原始程式碼。 請務必遵循讀我檔案中所概述的解除封鎖步驟。
...
1. 開啟 SSMS 並[連線到 tenants1-&lt;USER&gt; 伺服器](saas-dbpertenant-wingtip-app-guidance-tips.md#explore-database-schema-and-execute-sql-queries-using-ssms)。
...
現在嘗試[租用戶分析教學課程](saas-tenancy-tenant-analytics.md)，了解如何將資料擷取到另一個分析資料庫，進行更複雜的分析處理...
