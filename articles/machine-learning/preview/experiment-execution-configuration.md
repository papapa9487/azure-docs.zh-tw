---
title: "Azure Machine Learning 實驗執行服務概觀"
description: "本文件提供 Azure Machine Learning 實驗執行服務的整體概觀"
...
實驗執行服務的建置目的是提供隔離、重現問題，以及一致執行您的實驗。 它可協助您管理計算目標、執行環境和回合組態。 您可以使用 Azure ML Workbench 執行和執行管理功能，輕鬆地在不同環境之間移動。 

您可以在 Azure ML Workbench 專案中以本機方式，或在雲端中大量執行 Python 或 PySpark 指令碼。 

您可以在以下位置執行指令碼： 

* Azure ML Workbench 安裝在本機電腦上的 Python (3.5.2) 環境。
...
>[!IMPORTANT]
>Azure ML 執行服務目前分別支援 Python 3.5.2 和 Spark 2.1.11 作為 Python 和 Spark 執行階段版本。 


## <a name="key-concepts-in-azure-ml-experiment-execution"></a>Azure ML 實驗執行中的重要概念
請務必在 Azure ML 實驗執行中了解下列概念。 在後續章節中，我們會詳細討論如何使用這些概念。 
### <a name="compute-target"></a>計算目標
計算目標會指定要執行使用者程式的位置，例如使用者的桌面、遠端 Docker VM 或叢集。 計算目標必須可供使用者定址且可存取。 Azure ML Workbench 讓您能夠使用 Workbench 應用程式和 CLI 來建立計算目標並加以管理。 

CLI 中的 _az ml computetarget attach_ 命令可讓您建立能用於執行中的計算目標。

### <a name="supported-compute-targets-are"></a>支援的計算節點包括：
* Azure ML Workbench 安裝在電腦上的本機 Python (3.5.2) 環境。
* 電腦上的本機 Docker
...
執行環境會定義執行時間組態和 Azure ML Workbench 中執行程式所需的相依性。

如果是在 Azure ML Workbench 預設的執行階段上執行，使用者就會使用他們最愛的工具和套件管理員來管理本機執行環境。 

Conda 是用來管理本機 Docker 和遠端 Docker 執行和以 HDInsigh 作為基礎的執行。 針對這些計算目標，執行環境組態是透過 **Conda_dependencies.yml** 和 **Spark_dependencies.yml files** 來管理。 這些檔案是位於專案內的 **aml_config** 資料夾。
...
除了計算目標和執行環境，Azure ML 還會提供一個架構，可定義及變更回合組態。 每次執行實驗可能需要不同組態作為反覆試驗的一部分。 您可能要清除不同的參數範圍、使用不同的資料來源及調整 Spark 參數。 Azure ML 執行服務所提供的架構可用來管理回合組態。
...
Azure ML Workbench 中的回合組態也可讓您指定環境變數。 您可以指定環境變數，並在程式碼中加以使用，方法是在 .runconfig 檔案中新增下一節。 
...
## <a name="azure-ml-experiment-execution-scenarios"></a>Azure ML 實驗執行情節
在本節中，我們要深入探討執行情節，並深入了解 Azure ML 會如何在遠端 VM 及在 HDInsight 叢集上執行實驗，特別是在本機執行實驗。 本節是從建立計算目標開始到執行您實驗的逐步解說。

>[!NOTE]
>本文章的其餘部分中，我們要使用 CLI (命令列介面) 命令來顯示概念和功能。 也可從 Workbench 桌面應用程式使用此處所述的功能。

## <a name="launching-the-cli"></a>啟動 CLI
啟動 CLI 的簡單方法是在 Azure ML Workbench 桌面應用程式中開啟專案，並導覽至 [檔案] --> [開啟命令提示字元]。
...
>[!NOTE]
> 當您從命令視窗中執行任何 _az ml_ 命令時，需要針對 Azure 進行驗證。 CLI 會使用獨立的驗證快取，然後是桌面應用程式，因此登入 Workbench 桌面應用程式並不表示已在 CLI 環境中進行驗證。 如需進行驗證，請依照下列步驟執行。 驗證權杖會在本機快取一段時間，讓您只在權杖過期時，才需要重複這些步驟。 當權杖到期或您看到驗證錯誤時，請執行下列命令：
...
>[!NOTE] 
>當您在專案資料夾內執行 _az ml_ 命令時，請確認專案屬於_目前_ Azure 訂用帳戶上的 Azure ML 測試帳戶。 否則可能會發生執行錯誤。


## <a name="running-scripts-and-experiments"></a>執行指令碼和實驗
您可以透過 Azure ML Workbench，使用 _az ml experiment submit_ 命令在各種計算目標上執行您的 Python 和 PySpark 指令碼。 此命令需要回合組態定義。 

當您建立計算目標時，Azure ML Workbench 會建立對應的 .runconfig 檔案，但您可以使用 _az ml runconfiguration  create_ 命令來建立其他的回合組態。 您也可以手動編輯回合組態檔。

回合組態會在 Workbench 應用程式中顯示作為實驗執行體驗的一部分。 

>[!NOTE]
>您可以在[實驗執行組態參考](experiment-execution-configuration-reference.md)一節中進一步了解回合組態檔。

## <a name="running-a-script-locally-on-azure-ml-workbench-installed-runtime"></a>在安裝 Azure ML Workbench 的執行階段上本機執行指令碼
Azure ML Workbench 可讓您對安裝 Azure ML Workbench 的 Python 3.5.2 執行階段直接執行指令碼。 會在 Azure ML Workbench 安裝時間安裝這個預設的執行階段，並包含 Azure ML 程式庫和相依性。 本機執行的執行結果與構件仍會儲存在雲端中的執行歷程記錄服務。

不同於以 Docker 作為基礎的執行，此組態_不_是由 Conda 所管理。 必須手動佈建您本機 Azure ML Workbench Python 環境的套件相依性。
...
您可以在 Azure ML Workbench CLI 視窗中輸入下列命令，找到預設 Python 環境的路徑。
```
$ conda env list
```

>[!NOTE]
>目前**不**支援直接針對本機 Spark 環境執行 PySpark。 Azure ML Workbench 支援在本機 Docker 上執行的 PySpark 指令碼。 Azure ML 基礎 Docker 映像隨附於 Spark 2.1.11 預先安裝。 
...
您也可以透過 Azure ML 執行服務，在本機電腦上的 Docker 容器執行您的專案。 Azure ML Workbench 提供的基礎 Docker 映像隨附於 Azure ML 程式庫以及 Spark 2.1.11 執行階段，以方便本機 Spark 執行。 Docker 必須已在本機電腦上執行。
...
已使用 Azure ML 基礎 Docker 映像準備好本機 Docker 上的執行環境。 第一次執行並利用使用者的 conda_dependencies.yml 檔案中指定的套件重疊時，Azure ML Workbench 會下載此映像。 這項作業會使初始執行較慢，但是受惠於 Workbench 重複使用快取的層，後續執行速度會大幅提升。 
...
在某些情況下，使用者本機電腦上可用的資源可能不夠定型所需的模型。 在此情況下，Azure ML 執行服務可提供輕鬆的方式，使用遠端 Docker 執行在更強大的 VM 上執行 Python 或 PySpark 指令碼。 
...
## <a name="running-a-script-on-hdinsight-cluster"></a>在 HDInsight 叢集上執行指令碼
HDInsight 是支援 Apache Spark 的巨量資料分析常用平台。 Azure ML Workbench 可讓您使用 HDInsight Spark 叢集實驗巨量資料。 
...
Azure ML Workbench 會使用 Conda 來準備及管理 HDInsight 叢集上的執行環境。 組態是由 _conda_dependencies.yml_ 和 _spark_dependencies.yml_ 組態檔所管理。 

使用者需要 HDInsight 叢集的 SSH 存取權，才能在此模式中執行實驗。 
