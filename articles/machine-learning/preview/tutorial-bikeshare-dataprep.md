...
1. 按下 __Ctrl__ 鍵，同時選取資料行標頭，可讓您一次選取多個資料行。 使用此選項以選取下列資料行標頭：
   * **HOURLYDRYBULBTEMPF**
   * **HOURLYRelativeHumidity**
   * **HOURLYWindSpeed**
...
1. 複選 **start station latitude** 和 **start station longitude** 資料行。 以滑鼠右鍵按一下其中一個資料行，然後選取 [地圖]。
    ...
    > [!TIP]
    > 若要啟用複選，請按住 __Ctrl__ 鍵，並選取每個資料行的標頭。
...
3. 若要移除 **Date Hour Range** 和 **rDate Hour Range** 資料行，請按下 **Ctrl** 鍵，並選取每個資料行標頭。 按一下滑鼠右鍵，然後選取 [移除資料行]。
