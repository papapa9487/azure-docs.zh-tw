...
此文章已被取代。 此討論與目前的問題 (亦即航太工業中的預測性維護) 仍有相關，但請參閱[商務對象適用的解決方案概觀](https://github.com/Azure/cortana-intelligence-predictive-maintenance-aerospace) \(英文\) 了解最新資訊。

## <a name="acknowledgements"></a>**通知**
本文是由 Microsoft 的資料科學家 Yan Zhang、Gauher Shaheen、Fidan Boylu Uz 和軟體工程師 Dan Grecoe 所共同撰寫。

## <a name="overview"></a>**概觀**
解決方案範本的設計是要加速在 Cortana Intelligence 套件之上建置 E2E 示範的程序。 已部署的範本會以所需的 Cortana Intelligence 元件佈建您的訂用帳戶，並建立它們之間的關聯性。 它也會在資料管線內植入由資料產生器應用程式所產生的範例資料，供您在部署解決方案範本之後下載並安裝到本機電腦上。 由產生器產生的資料會產生資料管線，並開始產生機器學習服務預測，然後就可以在 Power BI 儀表板上將它們視覺化。 部署程序將引導您完成數個步驟來設定解決方案的認證。 務必記錄這些認證，例如您在部署期間提供的解決方案名稱、使用者名稱和密碼。  

這份文件的目標在於說明參考架構與隨著此解決方案範本佈建在您訂用帳戶中的元件、說明如何用自己的資料取代範例資料，以及說明如何修改解決方案範本。  
...
部署解決方案會啟動 Cortana Analytics Suite 內的 Azure 服務 (事件中樞、串流分析、HDInsight、Data Factory、Machine Learning 等)。 架構圖顯示為航太解決方案範本建構預測性維護的方式。 您可以在 Azure 入口網站中調查這些服務，方法是對部署解決方案時所建立之解決方案範本圖表上的服務按一下 (但 HDInsight 除外，因為當需要相關的管線活動才能執行時才會隨選佈建 HDInsight，並且會於之後刪除)。
...
使用 [Azure 串流分析](https://azure.microsoft.com/services/stream-analytics/)服務為來自 [Azure 事件中樞](#azure-event-hub)服務的輸入串流提供近乎即時的分析，並將結果發佈到 [Power BI](https://powerbi.microsoft.com) 儀表板，以及將所有未經處理的內送事件封存至 [Azure 儲存體](https://azure.microsoft.com/services/storage/)服務，供 [Azure Data Factory](https://azure.microsoft.com/documentation/services/data-factory/) 服務後續處理。
...
使用 [Azure SQL Database](https://azure.microsoft.com/services/sql-database/) 儲存 (由 Azure Data Factory 管理) Azure Machine Learning 服務收到的預測，在 [Power BI](https://powerbi.microsoft.com) 儀表板中取用。

## <a name="data-consumption"></a>**資料耗用量**
### <a name="power-bi"></a>Power BI
使用 [Power BI](https://powerbi.microsoft.com) 服務顯示儀表板，其中包含 [Azure 串流分析](https://azure.microsoft.com/services/stream-analytics/)服務提供的彙總與警示，以及儲存在 [Azure SQL Database](https://azure.microsoft.com/services/sql-database/) 中使用 [Azure Machine Learning](https://azure.microsoft.com/services/machine-learning/) 服務產生的 RUL 預測。
...
下列各節將討論引進新資料集時將需要修改的範本區段。

### <a name="azure-event-hub"></a>Azure 事件中樞
Azure 事件中樞服務非常廣泛；資料可以 CSV 或 JSON 格式張貼至中樞。 Azure 事件中樞中未發生任何特殊處理，但務必了解提供給它的資料。
...
請確定您在未積極使用解決方案時有停止資料產生器，因為執行資料產生器將會產生較高的成本。 如果不使用解決方案，請將其刪除。 刪除解決方案時，會將您在部署解決方案時，於訂用帳戶中佈建的所有元件刪除。 如果要刪除解決方案，請在解決方案範本左側面板中按一下該解決方案的名稱，然後按一下 [刪除]。
