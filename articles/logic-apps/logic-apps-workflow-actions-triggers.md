...
|-----------------|----------------|------------|  
|200|無\(\)|執行工作流程，並再次檢查是否有其他內容遵循定義循環。|  
|200|10|執行工作流程，並在 10 秒後再次檢查是否有其他內容。|  
|202|60|不會觸發工作流程。 下一次嘗試會在一分鐘內開始，取決於定義循環。 如果定義循環不超過一分鐘，則 Retry-After 標頭的優先順序較高。 否則會遵循定義循環。|  
|400|無\(\)|不正確的要求，不會執行工作流程。 如果未定義任何**重試原則**，則會使用預設原則。 達到重試次數之後，觸發程序將再次檢查是否有其他遵循定義循環的內容。|  
|500|無\(\)|伺服器錯誤，不會執行工作流程。  如果未定義任何**重試原則**，則會使用預設原則。 達到重試次數之後，觸發程序將再次檢查是否有其他遵循定義循環的內容。|  
