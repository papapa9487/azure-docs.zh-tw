...
description: "了解複本、執行個體、其功能及生命週期"
...
一旦叢集資源管理員決定執行個體的位置後，它就會進入此生命週期的狀態。 此時，執行個體將在節點上啟動 (應用程式主機已啟動，執行階段已建立並開啟)。 在啟動後完成，執行個體會轉換成就緒狀態。 

如果應用程式主機或此執行個體的節點當機，便會轉換成已卸除的狀態。

### <a name="ready-rd"></a>就緒 (RD)
在就緒狀態中，執行個體在節點上已啟動並執行中。 如果這是個可靠服務，便已叫用 RunAsync。 

如果應用程式主機或此執行個體的節點當機，便會轉換成已卸除的狀態。

### <a name="closing-cl"></a>關閉中 (CL)
在關閉中的狀態，Service Fabric 正在關閉此節點上執行個體的流程。 有許多種原因會造成此狀況，例如應用程式升級、負載平衡或正在刪除服務。 關機完成之後，會轉換成已卸除狀態。

### <a name="dropped-dd"></a>已卸除 (DD)
在已卸除狀態中，執行個體不會於節點上執行。 此時，Service Fabric 正在維護此執行個體相關的中繼資料 (將會延遲刪除)。
...
####<a name="primary-inbuild-replicas"></a>主要建置中複本 
主要建置中複本是分割區的第一個複本。 這通常發生分割區正在建立時。 當分割區所有的複本重新啟動或已卸除時，也會引發主要建置中複本。

####<a name="idlesecondary-inbuild-replicas"></a>IdleSecondary 建置中複本
這些複本是叢集資源管理員正在建立的新複本， 也可能為已關閉且必須新增回複本集中的現有複本。 這些複本在由主要複本植入或建置後，才能夠加入複本集作為 ActiveSecondary，並參與作業的仲裁認知。

####<a name="activesecondary-inbuild-replicas"></a>ActiveSecondary 建置中複本
此狀態會在某些查詢中觀察到。 它是複本集未變更，但必須建置複本的最佳化狀況。 複本本身依照正常的狀態機器轉換 (如複本角色一節中所述)。
...
- **正在關閉複本的程式碼**：Service Fabric 可能需要關閉複本的執行中程式碼。 有許多原因會造成此狀況，例如應用程式、網狀架構或基礎結構升級，或因為複本回報錯誤等等。當複本關閉完成時，複本會轉換成關閉狀態。 與儲存在磁碟上的這個複本相關聯的持續性狀態不會清除。

- **將複本從叢集中移除**：Service Fabric 可能需要移除持續性狀態，並關閉複本執行中的程式碼。 有許多原因可能造成此狀況，例如負載平衡

### <a name="dropped-dd"></a>已卸除 (DD)
在已卸除狀態中，執行個體不會於節點上執行。 在節點上也沒有留下狀態。 此時，Service Fabric 正在維護此執行個體相關的中繼資料 (最終也會刪除)。
...
當 Service Fabric 必須將複本再次啟動時，關閉的複本會進入開啟中狀態。 例如，這可能在應用程式在節點上完成程式碼更新後發生。 
...
待命複本是已關閉然後開啟之持續性服務的複本。 Service Fabric 如果必須將另一個複本新增至複本集時，會使用此複本 (因為它已經有狀態的一部分，可加速建置流程)。 StandByReplicaKeepDuration 到期之後，會捨棄待命複本。
...
- **ActiveSecondary (S)**：這些是從主要複本接收狀態更新、套用更新並傳送認知的複本。 複本集中會有多個使用中次要複本，這些次要複本的數目會決定服務可處理的錯誤數目。
- **IdleSecondary (I)**：這些複本正由主要複本建立中，須先接收來自主要複本的狀態，才能夠升階至使用中次要複本。 
- **無 (N)**：這些複本在複本集沒有責任。
- **未知 (U)**：這是複本在從 Service Fabric 接收任何 ChangeRole API 呼叫前的初始角色。

下節說明複本角色轉換及一些轉換會發生的範例情節：
...
- U -> I：建立新的閒置複本
- U -> N：刪除待命複本
- I -> S：將閒置次要複本升階為使用中次要複本，讓其認知對仲裁有貢獻。
- I -> P：將閒置次要複本升階為主要複本。 這會在閒置次要複本為主要複本的正確候選對象時，於特殊重新設定下發生。
- I -> N：刪除閒置次要複本。
- S -> P：將使用中次要複本升階至主要複本。 這是因為叢集資源管理員起始主要複本容錯移轉或主要複本移動，以回應應用程式升級或負載平衡等等情況。
- S -> N：刪除使用中次要複本。
- P -> S：升階主要複本。 這可能因為叢集資源管理員起始主要複本移動發生，以回應應用程式升級或負載平衡等等。
- P -> N：刪除主要複本
