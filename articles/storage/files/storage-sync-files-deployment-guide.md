...
> [!Important]  
> 您可以變更同步群組中的任何雲端或伺服器端點，並將您的檔案與同步群組中的其他端點同步。 如果直接對雲端端點 (Azure 檔案共用) 進行變更，請注意，變更必須先經過雲端端點每 24 小時僅起始一次的 Azure 檔案同步變更偵測工作來探索。 如需詳細資訊，請參閱 [Azure 檔案服務常見問題集](storage-files-faq.md#afs-change-detection)。
