...
* **問：什麼是 Azure 檔案服務？**  
    ...
    Azure 檔案服務是 Azure 中的分散式檔案系統。 其所提供的 SMB 通訊協定介面可讓使用者將儲存體當做原生共用，掛接到支援的 Azure 虛擬機器或內部部署機器上。

* **問：Azure 檔案服務為何很實用？**  
   
    Azure 檔案服務可跨多個 VM 和平台存取共用資料。 請參閱 [Azure 檔案服務為何很實用](storage-files-introduction.md#why-azure-files-is-useful)。

* **問：何時應該使用 Azure 檔案、Azure Blob 或 Azure 磁碟？**  
   
    Microsoft Azure 提供在雲端中儲存及存取資料的幾種方式。  
   
    Azure 檔案服務 - 提供 SMB 介面、用戶端程式庫和 REST 介面，允許從任何位置輕鬆存取儲存的檔案。  
...
* **問：如何開始使用 Azure 檔案服務？**  
    ...
    * [了解如何使用入口網站建立 Azure 檔案共用](storage-how-to-use-files-portal.md)
...
* **問：Azure 檔案服務支援何種複寫？**  
    ...
    Azure 檔案服務目前只支援 LRS 或 GRS。 我們打算支援 RA-GRS，但時程尚未決定。
    
## <a name="scale-targetslimits"></a>調整目標/限制
...
* **問：如何對 Azure 檔案服務啟用伺服器端加密？**

    Azure 檔案服務的[伺服器端加密](../common/storage-service-encryption.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json)在所有區域和公用與國內雲端公開上市。 您可以使用 [Azure 入口網站](https://portal.azure.com/)、[Microsoft Azure 儲存體資源提供者 API](/rest/api/storagerp/storageaccounts)、[Azure PowerShell](https://msdn.microsoft.com/library/azure/mt607151.aspx) 或 [Azure CLI](../common/storage-azure-cli.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json)，對 Azure 檔案服務啟用 SSE。
    
    對 Azure 檔案服務啟用 SSE 之後，在該儲存體帳戶中寫入檔案儲存體的所有新資料將會自動加密。 這項功能適用於現有或新的儲存體帳戶中，寫入現有或新共用的所有新資料。 此用此功能不會額外收費。 深入了解[如何對 Azure 檔案服務啟用 SSE](../common/storage-service-encryption.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json)。

* **問：Azure 檔案服務是否支援以 Active Directory 為基礎的驗證？**
...
* **問：必須使用 Azure ExpressRoute 從內部部署 VM 連線到 Azure 檔案服務嗎？**
   
    否。 如果沒有 ExpressRoute，您仍然可以從內部部署存取檔案共用，只要將連接埠 445 (TCP 輸出) 開啟供網際網路存取即可。 不過，想要的話，ExpressRoute 也可以搭配 Azure 檔案服務一起使用。
...
* **問：嘗試將檔案解壓縮到 Azure 檔案服務時，執行速度很緩慢。我該怎麼辦？**
    
    若要將大量檔案傳輸到 Azure 檔案，建議您使用 AzCopy (Windows、Linux/Unix 預覽版) 或 Azure PowerShell，因為這些工具已針對網路傳輸最佳化。

* **問：已發行哪些修補程式來修正 Azure 檔案服務的效能變慢問題？**
    
    Windows 小組最近已發行修補程式，可修正當客戶從 Windows 8.1 或 Windows Server 2012 R2 存取 Azure 檔案服務時效能變慢的問題。 如需詳細資訊，請參閱相關的知識庫文章：[當您從 Windows 8.1 或 Server 2012 R2 存取 Azure 檔案服務時效能變慢](https://support.microsoft.com/kb/3114025)。

## <a name="features-and-interoperability-with-other-services"></a>與其他服務的功能互通性
* **問：容錯移轉叢集的「檔案共用見證」是否為 Azure 檔案服務的其中一個使用案例？**
...
* **問：如何針對 Azure 檔案服務錯誤進行疑難排解？**
    
    您可以參考 [Azure 檔案疑難排解文章](storage-troubleshoot-windows-file-connection-problems.md)以取得端對端疑難排解指引。 
...
* [針對 Azure 檔案服務問題進行疑難排解](storage-troubleshoot-linux-file-connection-problems.md)
...
* [將資料移轉至 Azure 檔案服務](https://azure.microsoft.com/blog/migrating-data-to-microsoft-azure-files/)
