# 概觀
## [開發人員指南](azure-developer-guide.md) 

## [Azure 服務概觀](https://docs.microsoft.com/azure/fundamentals-introduction-to-azure)
## [入口網站概觀](https://docs.microsoft.com/azure/azure-portal-overview)
## [SDK 及工具](https://docs.microsoft.com/azure/index#pivot=sdkstools)

# 快速入門
## [Web Apps](https://docs.microsoft.com/azure/app-service-web/app-service-web-get-started-html)
## 虛擬機器
### [Linux](https://docs.microsoft.com/azure/virtual-machines/linux/quick-create-portal)
### [Windows](https://docs.microsoft.com/azure/virtual-machines/windows/quick-create-portal)
## [無伺服器](https://docs.microsoft.com/azure/azure-functions/functions-create-first-azure-function)
## 微服務
### [Service Fabric](https://docs.microsoft.com/azure/service-fabric/service-fabric-create-your-first-application-in-visual-studio)
### [容器服務](https://docs.microsoft.com/azure/container-service/dcos-swarm/container-service-deployment) 

# 教學課程
## 建立和部署 Web 應用程式
### [.NET with SQL DB](https://docs.microsoft.com/azure/app-service-web/app-service-web-tutorial-dotnet-sqldatabase)
### [Node.js with Mongo DB](https://docs.microsoft.com/azure/app-service-web/app-service-web-tutorial-nodejs-mongodb-app)
### [PHP with MySQL](https://docs.microsoft.com/azure/app-service-web/app-service-web-tutorial-php-mysql)
### [Java with MySQL](https://docs.microsoft.com/azure/app-service-web/app-service-web-tutorial-java-mysql)
## 部署複雜的 VM 範本
### [Linux](https://docs.microsoft.com/azure/virtual-machines/linux/tutorial-manage-vm)
### [Windows](https://docs.microsoft.com/azure/virtual-machines/windows/tutorial-manage-vm)
## [建立連接 Azure 的函式](https://docs.microsoft.com/azure/azure-functions/functions-create-an-azure-connected-function)
## [在 Linux 上部署 Web 應用程式](https://docs.microsoft.com/azure/app-service/containers/quickstart-custom-docker-image) 

# 範例
## Azure CLI
### [Web Apps](https://docs.microsoft.com/azure/app-service-web/app-service-cli-samples)
### [Linux VM](https://docs.microsoft.com/azure/virtual-machines/linux/cli-samples)
### [Windows VM](https://docs.microsoft.com/azure/virtual-machines/windows/cli-samples)
## PowerShell
### [Web Apps](https://docs.microsoft.com/azure/app-service-web/app-service-powershell-samples)
### [Linux VM](https://docs.microsoft.com/azure/virtual-machines/linux/powershell-samples)
### [Windows VM](https://docs.microsoft.com/azure/virtual-machines/windows/powershell-samples)

# 概念
## [帳單及訂用帳戶](https://docs.microsoft.com/azure/billing/billing-getting-started)
## [主機比較](https://docs.microsoft.com/azure/app-service-web/choose-web-site-cloud-service-vm)
## [什麼是 App Service？](https://docs.microsoft.com/azure/app-service/app-service-value-prop-what-is)
## 虛擬機器
### [Linux VM](https://docs.microsoft.com/azure/virtual-machines/linux/overview)
### [Windows VM](https://docs.microsoft.com/azure/virtual-machines/windows/overview)
## [Service Fabric 概觀](https://docs.microsoft.com/azure/service-fabric/service-fabric-overview)

# 使用說明指南
## [規劃](https://docs.microsoft.com/azure/architecture/guide/)
### [Web 應用程式架構](https://docs.microsoft.com/azure/architecture/reference-architectures/managed-web-app/)
### [VM 架構](https://docs.microsoft.com/azure/architecture/reference-architectures/virtual-machines-windows/)
### [連線到內部部署網路](https://docs.microsoft.com/azure/architecture/reference-architectures/hybrid-networking/)
### [微服務模式/案例](https://docs.microsoft.com/azure/service-fabric/service-fabric-patterns-and-scenarios)
## 開發
### [Linux VM](https://docs.microsoft.com/azure/virtual-machines/linux/create-ssh-secured-vm-from-template)
### [Windows VM](https://docs.microsoft.com/azure/virtual-machines/windows/ps-template)
### [無伺服器應用程式](https://docs.microsoft.com/azure/azure-functions/functions-reference)
### [微服務叢集](https://docs.microsoft.com/azure/service-fabric/service-fabric-cluster-creation-via-portal)
## 部署
### [從原始檔控制 Web 和行動裝置應用程式](https://docs.microsoft.com/azure/app-service-web/app-service-continuous-deployment)
### [本機微服務](https://docs.microsoft.com/azure/service-fabric/service-fabric-get-started-with-a-local-cluster)
### [Linux VM](https://docs.microsoft.com/azure/virtual-machines/linux/tutorial-manage-vm)
### [Windows VM](https://docs.microsoft.com/azure/virtual-machines/windows/tutorial-manage-vm)
## 儲存資料
### [Blobs (英文)](https://docs.microsoft.com/azure/storage/blobs/storage-dotnet-how-to-use-blobs)
### [檔案共用](https://docs.microsoft.com/azure/storage/files/storage-dotnet-how-to-use-files)
### [機碼值組](https://docs.microsoft.com/azure/cosmos-db/table-storage-how-to-use-dotnet)
### [JSON 文件](https://docs.microsoft.com/azure/cosmos-db/documentdb-get-started)
### [關聯式資料表](https://docs.microsoft.com/azure/sql-database/sql-database-get-started)
### [訊息佇列](https://docs.microsoft.com/azure/storage/queues/storage-dotnet-how-to-use-queues)

## 調整
### [Web 和行動裝置應用程式](https://docs.microsoft.com/azure/app-service-web/web-sites-scale)
### [虛擬機器](https://docs.microsoft.com/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-overview)
### [微服務應用程式](https://docs.microsoft.com/azure/service-fabric/service-fabric-concepts-scalability)

## 安全
### [Web 和行動裝置應用程式](https://docs.microsoft.com/azure/app-service-web/app-service-web-tutorial-custom-ssl)

## 備份
### [Web 和行動裝置應用程式](https://docs.microsoft.com/azure/app-service-web/web-sites-backup)
### [虛擬機器](https://docs.microsoft.com/azure/backup/backup-azure-vms-introduction)

## 監視
### [Web 和行動裝置應用程式](https://docs.microsoft.com/azure/app-service-web/web-sites-enable-diagnostic-log)
### [Linux VM](https://docs.microsoft.com/azure/virtual-machines/linux/tutorial-monitoring)
### [Windows VM](https://docs.microsoft.com/azure/virtual-machines/windows/tutorial-monitoring)
### [微服務](https://docs.microsoft.com/azure/service-fabric/service-fabric-diagnostics-overview)
### [帳單警示](https://docs.microsoft.com/azure/billing/billing-set-up-alerts)

## 自動化
### [敏捷式 Web 應用程式部署](https://docs.microsoft.com/azure/app-service-web/app-service-agile-software-development)
### [調整 Linux VM](https://docs.microsoft.com/azure/virtual-machines/linux/tutorial-create-vmss)
### [調整 Windows VM](https://docs.microsoft.com/azure/virtual-machines/windows/tutorial-create-vmss)

# 參考
## [REST](https://docs.microsoft.com/rest/api/)
## SDK
### [.NET](https://go.microsoft.com/fwlink/?linkid=834925)
### [Java](https://docs.microsoft.com/java/api/)
### [Node.js](http://azure.github.io/azure-sdk-for-node/)
### [PHP](https://github.com/Azure/azure-sdk-for-php/blob/master/README)
### [Python](http://azure-sdk-for-python.readthedocs.io/en/latest/)
### [Ruby](https://github.com/Azure/azure-sdk-for-ruby/blob/master/README)
## 命令列介面
### [Azure CLI2.0](https://docs.microsoft.com/cli/azure/)
### [PowerShell](https://docs.microsoft.com/powershell/)
## [計費](https://docs.microsoft.com/azure/billing/billing-usage-rate-card-overview)

# 資源
## [Azure 區域](https://azure.microsoft.com/regions/)
## [Azure 藍圖](https://azure.microsoft.com/roadmap/)
## [定價計算機](https://azure.microsoft.com/pricing/calculator/)
## [範例](https://azure.microsoft.com/resources/samples/)
## [影片](https://azure.microsoft.com/resources/videos/home/)
