...
若要讓單一登入運作，Azure AD 必須知道 QPrism 與 Azure AD 中互相對應的使用者。 換句話說，必須建立 Azure AD 使用者和 QPrism 中相關使用者之間的連結關聯性。

在 QPrism 中，將 Azure AD 中**使用者名稱**的值，指派為 **Username** 的值，以建立連結關聯性。
...
1. 在 Azure 入口網站的 [QPrism] 應用程式整合頁面上，按一下 [單一登入]。
...
3. 在 [QPrism 網域與 URL] 區段中，執行下列步驟：
    ...
    b. 在 [識別碼] 文字方塊中，使用下列模式輸入 URL：`https://<customer domain>.qmyzone.com/metadata.php`
         
    > [!NOTE] 
    > 這些都不是真正的值。 請使用實際的識別碼和登入 URL 將這些值更新。 請連絡 [QPrism 用戶端支援小組](mailto:qsupport-ce@quatrro.com)以取得這些值。 

4. 若要產生**中繼資料** URL，執行下列步驟︰

    a. 按一下 [應用程式註冊]。
    
    ![設定單一登入 appreg](./media/active-directory-saas-qprism-tutorial/tutorial_qprism_appregistrations.png)
   
    b. 按一下 [端點] 以開啟 [端點] 對話方塊。  
    
    ![設定單一登入 endpointcon](./media/active-directory-saas-qprism-tutorial/tutorial_qprism_endpointicon.png)

    c. 按一下複製按鈕複製 [同盟中繼資料文件] URL，並將它貼到 [記事本]。
    
    ![設定單一登入 endpoint](./media/active-directory-saas-qprism-tutorial/tutorial_qprism_endpoint.png)
     
    d. 現在，移至 [QPrism] 的屬性頁面，使用 [複製] 按鈕複製 [應用程式識別碼]，並將它貼到記事本。
 
    ![設定單一登入 appid](./media/active-directory-saas-qprism-tutorial/tutorial_qprism_appid.png)
...
6. 若要在 **QPrism** 端設定單一登入，您必須將**中繼資料 URL** 傳送給 [QPrism 支援小組](mailto:qsupport-ce@quatrro.com)。 他們會進行此設定，讓兩端的 SAML SSO 連線都設定正確。
...
**若要將 Britta Simon 指派給 QPrism，請執行下列步驟：**
...
當您在存取面板中按一下 QPrism 圖示時，應該會自動登入 QPrism 應用程式。
