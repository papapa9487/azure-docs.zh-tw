...
- Azure AD 訂用帳戶
- 若要針對 Atlassian Cloud 產品啟用 SAML 單一登入，您必須設定「身分識別管理員」。 深入了解[身分識別管理員]( https://www.atlassian.com/enterprise/cloud/identity-manager) \(英文\)
...
將 Azure AD 中 [使用者名稱] 的值，指派為 Atlassian Cloud 中的 [使用者名稱] 值，以建立連結關聯性。

若要設定及測試與 Atlassian Cloud 搭配運作的 Azure AD 單一登入，您需要完成下列構成要素：

1. **[設定 Azure AD 單一登入](#configure-azure-ad-single-sign-on)**：讓您的使用者能夠使用此功能。
2. **[建立 Azure AD 測試使用者](#create-an-azure-ad-test-user)**：使用 Britta Simon 測試 Azure AD 單一登入。
3. **[建立 Atlassian Cloud 測試使用者](#create-an-atlassian-cloud-test-user)**：使 Atlassian Cloud 中具有與 Azure AD 中的 Britta Simon 連結的相對應使用者。
...
2. 在 [單一登入] 對話方塊上，於 [模式] 選取 [SAML 登入]，以啟用單一登入。
    ...
    ![單一登入對話方塊](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_samlbase.png)

3. 如果您想要以 **IDP** 起始模式設定應用程式，請在 [Atlassian Cloud 網域和 URL] 區段上執行下列步驟：

    ![[Atlassian Cloud 網域和 URL] 單一登入資訊](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_url_new.png)
    
    a. 在 [識別碼] 文字方塊中，輸入 URL：`https://id.atlassian.com/login`
    
    b. 在 [回覆 URL] 文字方塊中，輸入 URL：`https://id.atlassian.com/login/saml/acs`

    c. 在 [轉送狀態] 文字方塊中，使用下列模式輸入 URL：`https://<instancename>.atlassian.net`

4. 如果您想要以 **SP** 起始模式設定應用程式，請勾選 [顯示進階 URL 設定]，然後執行下列步驟：

    ![[Atlassian Cloud 網域和 URL] 單一登入資訊](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_url1.png)

    在 [登入 URL] 文字方塊中，使用下列模式輸入 URL︰`https://<instancename>.atlassian.net`

    > [!NOTE] 
    > 這些都不是真正的值。 您可以從本教學課程稍後將會說明的 [Atlassian Cloud SAML 設定] 畫面取得這些值。
...
7. 在 [Atlassian Cloud 設定] 區段中，按一下 [設定 Atlassian Cloud] 以開啟 [設定登入] 視窗。 從 [快速參考] 區段中複製 [SAML 實體 ID 和 SAML 單一登入服務 URL]。

    ![Atlassian Cloud 設定](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_configure.png) 

8. 若要取得針對您的應用程式設定的 SSO，請使用系統管理員權限登入 Atlassian 入口網站。

9. 瀏覽至 [Atlassian Site Administration] \(Atlassian 網站管理\) > [Organizations & Security] \(組織與安全性\)。 如果您還沒有建立並命名您的組織，請先這麼做。 然後，在左側導覽中，按一下 [Domains] \(網域\)。

    ![設定單一登入](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_06.png)

10. 選取您要驗證網域的方式：[DNS] 或 [HTTPS]。

    ![設定單一登入](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_17.png)

11. 若要使用 DNS 驗證，選取 [Domains] \(網域\) 頁面上的 [DNS] 索引標籤，並執行下列步驟：

    ![設定單一登入](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_18.png)

    a. 按一下 [Copy] \(複製\) 以複製 TXT 記錄的值。

    b. 從您的 DNS 中，尋找設定頁面以新增記錄

    c. 選取新增記錄的選項，並將您從 [Domains] \(網域\) 頁面所複製的值貼上到 [Value] \(值\) 欄位。 您的 DNS 也可能會將它稱為 [Answer] \(接聽\) 或 [Description] \(描述\)。
...
12. 返回組織管理中的 [Domains] \(網域\) 頁面，然後按一下 [Verify domain] \(驗證網域\) 按鈕。 在快顯視窗中輸入您的網域名稱，然後按一下 [Verify domain] \(驗證網域\) 按鈕。

    ![設定單一登入](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_19.png)
    
    > [!NOTE]
    > TXT 記錄變更可能需要最多 72 小時才會生效，您無法立即得知網域驗證是否成功。 在您執行這些步驟後，請檢查 [Domains] \(網域\) 頁面以取得驗證狀態。 您會看到下面的畫面，其中更新狀態已變成 [VERIFIED] \(已驗證\)。

    ![設定單一登入](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_20.png)

13. 若要使用 HTTPS 驗證，選取 [Domains] \(網域\) 頁面上的 [HTTPS] 索引標籤，並執行下列步驟：

    ![設定單一登入](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_21.png)

    a.  按一下 [Download file] \(下載檔案\) 以下載 HTML 檔案。

    b.  將 HTML 檔案上傳至網域的根目錄。

14. 返回組織管理中的 [Domains] \(網域\) 頁面，然後按一下 [Verify domain] \(驗證網域\) 按鈕。 在快顯視窗中輸入您的**網域名稱**，然後按一下 [Verify domain] \(驗證網域\) 按鈕。

    ![設定單一登入](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_22.png)

15. 如果驗證程序能夠找到您上傳到根目錄的檔案，則網域狀態將會更新為 [Verified] \(已驗證\)。

    ![設定單一登入](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_23.png)

    > [!NOTE]
    > 如需詳細資訊，請參閱 [Atlassian 的網域驗證文件](https://confluence.atlassian.com/cloud/domain-verification-873871234.html) \(英文\)

16. 在左側瀏覽列中，按一下 [SAML single sign-on] \(SAML 單一登入\)。 如果您還沒有訂閱 Atlassian 的身分識別管理員，請先這麼做。

    ![設定單一登入](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_11.png)

17. 在 [Add SAML configuration] \(新增 SAML 設定\) 對話方塊中，新增身分識別提供者設定，如下所示：
    ...
    c. 在記事本中開啟已下載的憑證，複製不含開始和結束行的值，並將它貼在 [Public X509 certificate] \(公用 X509 憑證\) 方塊中。
    
    d. 按一下 [Save Configuration] \(儲存設定\) 以儲存設定。
     
18. 更新 Azure AD 設定，確定您已設定正確的 URL。
  
    ![設定單一登入](./media/active-directory-saas-atlassian-cloud-tutorial/tutorial_atlassiancloud_13.png)

    a. 複製 SAML 畫面的 [SP Identity ID] \(SP 身分識別識別碼\)，將值貼入到 Azure 入口網站中的 [識別項] 方塊中 (位於 Atlassian Cloud [網域和 URL] 區段底下)。
    
    b. 複製 SAML 畫面的 [SP Assertion Consumer Service URL] \(SP 判斷提示取用者服務 URL\)，將值貼入到 Azure 入口網站中的 [回覆 URL] 方塊中 (位於 Atlassian Cloud [網域和 URL] 區段底下)。
    
    c. [登入 URL] 是 Atlassian Cloud 的租用戶 URL。 
    
19. 在 Azure 入口網站中，按一下 [儲存] 按鈕。
...
4. 在 [使用者] 對話方塊中，執行下列步驟：
    ...
    ![[使用者] 對話方塊](./media/active-directory-saas-atlassian-cloud-tutorial/create_aaduser_04.png)
