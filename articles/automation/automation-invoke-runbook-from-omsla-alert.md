...
*  您必須有將 WebhookData 轉換成 PowerShell 物件的程式碼。
    ...
    *$SearchResults* 是物件的陣列；每個物件都包含某個搜尋結果的值欄位
...
*  儲存該警示之後，警示會觸發並呼叫 Webhook，而傳送到 Runbook 的 WebhookData 包含 *.SearchResult*。
* 如果您建立警示並將其設定為呼叫 Runbook (這也會建立 Webhook)，警示觸發時，會將 WebhookData 傳送至內含 *.SearchResults* 的 Runbook。

因此在上述程式碼範例中，如果警示呼叫 Webhook，您必須取得 *.SearchResult*，而如果警示直接呼叫 Runbook，則必須取得 *.SearchResults*。

## <a name="example-walkthrough"></a>範例逐步解說

我們會透過下列範例圖形化 Runbook (其會啟動 Windows 服務) 示範其運作方式。<br><br> ![啟動 Windows 服務圖形化 Runbook](media/automation-invoke-runbook-from-omsla-alert/automation-runbook-restartservice.png)<br>
