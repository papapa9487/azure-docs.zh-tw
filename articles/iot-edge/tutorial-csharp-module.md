...
您可以使用 IoT Edge 模組來部署程式碼，將您的商務邏輯直接實作到您的 IoT Edge 裝置。 本教學課程會逐步引導您建立及部署 IoT Edge 模組，以篩選模擬 IoT Edge 裝置上的感應器資料 (模擬裝置是在＜在 [Windows][lnk-tutorial1-win] 或 [Linux][lnk-tutorial1-lin] 上的模擬裝置中部署 Azure IoT Edge＞教學課程中所建立)。 在本教學課程中，您將了解如何：    
...
您在本教學課程中建立的 IoT Edge 模組會篩選由您裝置產生的溫度資料，而且只會在溫度高於指定臨界值時，將訊息傳送到上游。 
...
* Visual Studio Code 的 Azure IoT Edge 功能
* [Docker](https://docs.docker.com/engine/installation/)。 在本教學課程中，使用 Community Edition (CE) 作為您的平台是可行的。 請確定您已在執行 VS Code 的電腦上安裝該項目。
* [.NET Core 2.0 SDK](https://www.microsoft.com/net/core#windowscmd)。 

## <a name="choose-or-sign-up-for-a-docker-registry"></a>選擇或註冊 Docker 登錄
在本教學課程中，您會使用 VS Code 的 Azure IoT Edge 擴充功能來建置模組，並建立 [Docker 映像](https://docs.docker.com/glossary/?term=image)。 然後將此 Docker 映像推送至裝載於 [Docker 登錄](https://docs.docker.com/glossary/?term=registry)上的 [Docker 存放庫](https://docs.docker.com/glossary/?term=repository)。 最後，您會將封裝為 [Docker 容器](https://docs.docker.com/glossary/?term=container) 的 Docker 映像從您的登錄部署至 IoT Edge 裝置。  
...
2. 在整合式終端機中，輸入下列命令來建立新的模組專案：
    ...
    >[!NOTE]
    > 此命令會在目前的工作資料夾中建立專案資料夾 **FilterModule**。 如果您想要建立在另一個位置中，請先變更目錄再執行此命令。
 
3. 使用**檔案 |開啟資料夾**功能表命令，瀏覽至 **FilterModule** 資料夾，然後按一下 [選取資料夾]，在 VS Code 中開啟專案。
4. 在 VS Code 檔案總管中，按一下 **Program.cs** 來將其開啟。
...
1. 將下列類別新增到 **Program** 類別。 這些類別會定義內送郵件本文的預期結構描述。
...
1. 在 **Init** 方法中，該程式碼會建立並設定 **DeviceClient** 物件。 此物件可允許模組連線至本機的 Azure IoT Edge 執行階段，以便傳送和接收訊息。 **Init** 方法中使用的連接字串參數，會由 **EdgeHubConnectionString** 環境變數中的 IoT Edge 執行階段提供給模組。 建立 **DeviceClient** 之後，程式碼會註冊回呼以透過 **input1** 端點接收來自 IoT Edge 中樞的訊息。 新方法會取代作為回呼以用來處理訊息的方法，並在模組對應項上附加兩個所需屬性更新的回呼。 若要進行這項變更，請使用下列程式碼取代 **Init** 方法的最後一行：
...
1. 將下列方法新增至 **Program** 類別，以根據後端服務透過模組對應項所傳送的所需屬性，來更新 **temperatureThreshold** 欄位。 所有模組都有自己的模組對應項。 模組對應項可讓後端服務設定正在模組內執行的程式碼。
...
1. 使用下列方法取代 **PipeMessage** 方法。 每當模組從 Edge 中樞傳送訊息時，就會呼叫此方法。 它會根據訊息本文中的溫度值，以及透過模組對應項設定的溫度臨界值，來篩選訊息。
...
11. 建置專案。 使用**檢視 | 檔案總管**功能表命令來開啟 VS Code 檔案總管。 在檔案總管中，以滑鼠右鍵按一下 **FilterModule.csproj** 檔案，然後按一下 [建置 IoT Edge 模組] 來編譯模組，並將二進位檔和其相依性匯出至下一個步驟中建立 Docker 映像的來源資料夾。

## <a name="create-a-docker-image-and-publish-it-to-your-registry"></a>建立 Docker 映像並發行到您的登錄中

1. 建置 Docker 映像。
    1. 在 VS Code 檔案總管中，按一下 [Docker] 資料夾來將其開啟。 然後為您的容器平台的選取資料夾，**linux-x64** 或 **windows-nano**。 
    2. 以滑鼠右鍵按一下 **Dockerfile** 檔案，然後按一下 [建置 IoT Edge 模組的 Docker 映像]。 
    3. 在 [選取資料夾] 方塊中，瀏覽或輸入 `./bin/Debug/netcoreapp2.0/publish`。 按一下 [選取資料夾為 EXE_DIR]。
    4. 在 VS Code 視窗頂端的快顯文字方塊中，輸入映像名稱。 例如：`<docker registry address>/filtermodule:latest`；其中「Docker 登錄位址」 是 Docker 識別碼 (如果您使用 Docker Hub)，或類似於 `<your registry name>.azurecr.io` (如果您使用 Azure Container Registry)。
...
## <a name="add-registry-credentials-to-edge-runtime-on-your-edge-device"></a>將登錄認證新增至 Edge 裝置上的 Edge 執行階段
...
3. 選取 [設定模組]。 
2. 新增 **tempSensor** 模組。 如果您先前沒有將 **tempSensor** 模組部署到 IoT Edge 裝置，才需要執行此步驟。
    1. 選取 [新增 IoT Edge 模組]。
...
9. 新增 **filtermodule**
    ...
    3. 在 [映像] 欄位中，輸入您的映像地址，例如`<docker registry address>/filtermodule:latest`。
    4. 請檢查 [編輯模組對應項] 方塊。
    ...
    6. 按一下 [儲存] 。
12. 按一下 [下一步] 。
13. 在 [指定路由] 步驟中，將下列 JSON 複製到文字方塊。 模組會將所有訊息發佈到 Edge 執行階段。 執行階段中的宣告式規則會定義這些訊息的流向。 在本教學課程中，您需要兩個路由。 第一個路由會透過使用 **FilterMessages**處理常式設定的 "input1" 端點，將訊息從溫度感應器傳輸至篩選模組。 第二個路由會將訊息從篩選模組傳輸到 IoT 中樞。 在此路由中，`upstream` 是告知 Edge 中樞將訊息傳送至 IoT 中樞的特殊目的地。 
