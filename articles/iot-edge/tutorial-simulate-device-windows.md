...
Azure IoT Edge 會將雲端的強大功能移至您的物聯網 (IoT) 裝置。 本教學課程會引導您建立產生感應器資料的模擬 IoT Edge 裝置。 您會了解如何：
...
IoT Edge 執行階段會在所有 IoT Edge 裝置上部署。 其包含兩個模組。 第一個 IoT Edge 代理程式可協助進行部署及監視 IoT Edge 裝置上的模組。 第二個 IoT Edge 中樞會管理 IoT Edge 裝置上的模組通訊，以及裝置與 IoT 中樞之間的通訊。 


使用下列步驟來安裝和啟動 IoT Edge 執行階段：

1. 使用上一節中的 IoT Edge 裝置連接字串來設定執行階段。
...
1. 檢查 Docker 以確認 IoT Edge 代理程式是否正作為模組執行中。
