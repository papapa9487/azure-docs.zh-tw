---
title: "如何使用用於容器的 Web 應用程式的 Maven 外掛程式，將容器化 Spring Boot 應用程式部署至 Azure"
description: "了解如何使用用於容器的 Web 應用程式的 Maven 外掛程式，將 Spring Boot 應用程式部署至 Azure。"
...
---
# <a name="how-to-use-the-maven-plugin-for-web-app-for-containers-to-deploy-a-containerized-spring-boot-app-to-azure"></a>如何使用用於容器的 Web 應用程式的 Maven 外掛程式，將容器化 Spring Boot 應用程式部署至 Azure

針對 [Apache Maven](http://maven.apache.org/) 的[用於容器的 Web 應用程式的 Maven 外掛程式](https://github.com/Microsoft/azure-maven-plugins/tree/master/azure-webapp-maven-plugin)提供 Azure App Service 到 Maven 專案的緊密整合，並且簡化開發人員將 Web 應用程式部署至 Azure App Service 的程序。

本文示範如何使用用於容器的 Web 應用程式的 Maven 外掛程式，將 Docker 容器中的範例 Spring Boot 應用程式部署至用於容器的 Web 應用程式。

> [!NOTE]
>
> 用於容器的 Web 應用程式的 Maven 外掛程式目前可供預覽。 雖然未來計劃有額外功能，但是現在僅支援 FTP 發佈。
>
...
## <a name="clone-the-sample-spring-boot-application"></a>複製 Spring Boot 應用程式範例
...
1. 將 [Spring Boot with Docker] 專案範例複製到您建立的目錄中；例如：
   ```shell
...
1. 將本教學課程上一節的 Azure 服務主體設定新增至 settings.xml 檔案中的 `<servers>` 集合；例如：
   ...
   `<environment>` | 定義目標 Azure 雲端環境，也就是此範例中的 `AZURE`。 (環境的完整清單可於[用於容器的 Web 應用程式的 Maven 外掛程式]文件中取得)
...
您可以為 Maven 外掛程式修改數個值，這些元素的詳細描述可於[用於容器的 Web 應用程式的 Maven 外掛程式]文件中取得。 也就是說，有數個值值得在這篇文章中反白顯示：

元素 | 說明
---|---|---
`<version>` | 指定[用於容器的 Web 應用程式的 Maven 外掛程式]版本。 您應該檢查 [Maven 中央存放庫](http://search.maven.org/#search%7Cga%7C1%7Ca%3A%22azure-webapp-maven-plugin%22)中所列的版本，確定您使用最新版本。
...
`<region>` | 指定目標區域，在此範例中是 `westus`。 (完整清單位於[用於容器的 Web 應用程式的 Maven 外掛程式]文件。)
...
* [用於容器的 Web 應用程式的 Maven 外掛程式]

* [從 Azure CLI 登入 Azure](/azure/xplat-cli-connect)

* [如何使用用於容器的 Web 應用程式的 Maven 外掛程式，將 Spring Boot 應用程式部署至 Linux 上的 Azure App Service](../app-service-web-deploy-spring-boot-app-with-maven-plugin.md)
...
[用於容器的 Web 應用程式的 Maven 外掛程式]: https://github.com/Microsoft/azure-maven-plugins/tree/master/azure-webapp-maven-plugin
