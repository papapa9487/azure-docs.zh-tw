...
> [!NOTE]
> 為獲得最佳結果，即使您想要使用已中斷連線的 Azure Stack 環境，最好是在已連線到網際網路時部署。 如此一來，就可以在部署期間啟動 Windows Server 2016 評估版。
> 
> 
...
3. 在 [類型] 方塊中，選取 [Azure 雲端] 或 [ADFS]。
    - **Azure 雲端**：Azure Active Directory 為識別提供者。 使用此參數可指定 AAD 帳戶具有全域系統管理員權限的特定目錄。 AAD Directory 租用戶的完整名稱。 例如，onmicrosoft.com。 
...
若要從 Powershell 變更密碼到期原則：
1. 從 Powershell 視窗執行命令：Set-ADDefaultDomainPasswordPolicy -MaxPasswordAge 180.00:00:00 -Identity azurestack.local
