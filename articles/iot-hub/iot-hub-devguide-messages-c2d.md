...
IoT 中樞上的**最大傳遞計數**屬性會決定訊息可以在**已加入佇列**與**不可見**狀態之間轉換的次數上限。 達到該轉換次數之後，IoT 中樞就會將訊息的狀態設定為 **寄不出**。 同樣地，IoT 中樞也會在訊息的到期時間過後 (請參閱[存留時間][lnk-ttl])，將訊息的狀態設定為**寄不出**。
...
| **negative** | 如果雲端到裝置訊息達到 [寄不出] 狀態，IoT 中樞就會產生意見反應訊息。 |
