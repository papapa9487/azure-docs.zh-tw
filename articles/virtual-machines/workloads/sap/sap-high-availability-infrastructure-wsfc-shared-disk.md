---
title: "使用 SAP (A)SCS 執行個體的 Windows 容錯移轉叢集和共用磁碟為 SAP HA 進行 Azure 基礎結構準備 | Microsoft Docs"
description: "使用 SAP (A)SCS 執行個體的 Windows 容錯移轉叢集和共用磁碟，為 SAP HA 進行 Azure 基礎結構準備"
...
---
# <a name="azure-infrastructure-preparation-for-sap-ha-using-windows-failover-cluster-and-shared-disk-for-sap-ascs-instance"></a>使用 SAP (A)SCS 執行個體的 Windows 容錯移轉叢集和共用磁碟，為 SAP HA 進行 Azure 基礎結構準備
...
本文件說明 Azure 基礎結構的準備步驟，需要在 **Windows 容錯移轉叢集 (WSFC)** 上安裝並設定高可用性的 SAP 系統，以**叢集共用磁碟**的形式將 SAP (A)SCS 執行個體轉為叢集。

## <a name="prerequisites"></a>必要條件

請務必先檢閱以下文件再開始安裝：

* [架構指南 - 使用**叢集共用磁碟**於 **Windows 容錯移轉叢集**上進行 SAP (A)SCS 執行個體叢集處理][sap-high-availability-guide-wsfc-shared-disk]

* [使用 **SAP (A)SCS** 執行個體的 **Windows 容錯移轉叢集**和**共用磁碟**，為 SAP HA 進行 Azure 基礎結構準備][sap-high-availability-infrastructure-wsfc-shared-disk]
...
## <a name="c87a8d3f-b1dc-4d2f-b23c-da4b72977489"></a> 使用公司網路連線能力 (跨單位) 部署虛擬機器，以用於生產環境中
...
## <a name="7fe9af0e-3cce-495b-a5ec-dcb4d8e0a310"></a> 部署用於測試和示範的僅限雲端 SAP 執行個體
...
若要設定資料庫多 SID 範本，請在[資料庫多 SID 範本][sap-templates-3-tier-multisid-db-marketplace-image]或[使用受控磁碟的資料庫多 SID 範本][sap-templates-3-tier-multisid-db-marketplace-image-md]中輸入下列參數的值︰
  ...
  -  **SAP 系統識別碼**。輸入您想要安裝的 SAP 系統之 SAP 系統識別碼。 此識別碼會作為所部署資源的前置詞。
  -  **OS 類型**。 選取虛擬機器的作業系統。
...
## <a name="661035b2-4d0f-4d31-86f8-dc0a50d78158"></a> 在 SAP ASCS/SCS 執行個體的兩個叢集節點上新增登錄項目
...
若要在 SAP ASCS/SCS 執行個體的兩個叢集節點上新增登錄項目，請先在 SAP ASCS/SCS 的兩個 Windows 叢集節點上新增下列 Windows 登錄項目︰
...
6.  將叢集虛擬主機名稱上線。
  ...
  ![圖 15：叢集核心服務以正確的 IP 位址啟動並正常執行][sap-ha-guide-figure-3014]

  _**圖 15：**叢集核心服務以正確的 IP 位址啟動並正常執行_
...
3.  若要將叢集名稱物件新增至清單中，請選取 [新增]。 變更篩選以檢查電腦物件 (除了上圖所示的項目以外)。
...
4.  輸入如**圖：變更物件類型以包含電腦**中所示的叢集名稱物件。 由於已建立記錄，因此您可變更權限，如**圖：為叢集名稱物件指派共用權限**。
...
5.  選取您要的變更，然後選取 [下一步]。 您必須成功重新設定叢集設定，如下所示：  

  ![圖 28：確認您已重新設定叢集][sap-ha-guide-figure-3027]

  _**圖 28：**確認您已重新設定叢集_

成功安裝 Windows 容錯移轉叢集之後，有些閾值需要變更，讓容錯移轉偵測適應 Azure 中的條件。 需要變更的參數記載於[調整容錯移轉叢集網路閥值][tuning-failover-cluster-network-thresholds]部落格中。 假設為 ASCS/SCS 建置 Windows 叢集組態的兩部 VM 位於相同子網路中，則必須將下列參數變更為這些值︰
...
1.  啟動 DataKeeper 管理與組態工具，然後選取 [連線伺服器]。 (此選項是用紅色圈起來的部分。)
...
5.  定義名稱、TCP/IP 位址和目標節點的磁碟區。
  ...
  ![圖 41：定義節點 (應該是目前的目標節點) 的基底資料][sap-ha-guide-figure-3041]

  _**圖 41：**定義節點 (應該是目前的目標節點) 的基底資料_

6.  定義壓縮演算法。 在我們的範例中，建議壓縮複寫資料流。 尤其是在重新同步處理的情況下，壓縮複寫資料流可大幅縮短重新同步處理時間。 壓縮會使用虛擬機器的 CPU 和 RAM 資源。 隨著壓縮率增加，使用的 CPU 資源數量也跟著增加。 您也可以稍後調整此設定。
...
8.  定義是否應該向 Windows Server 容錯移轉叢集的叢集組態指出複寫作業所複寫的磁碟區是共用磁碟。 就 SAP ASCS/SCS 組態而言，選取 [是] ，如此 Windows 叢集才會將複寫的磁碟區視為可用來作為叢集磁碟區的共用磁碟。
  ...
  「容錯移轉叢集管理員」現在會顯示磁碟為 DataKeeper 磁碟，如下所示：
...
* [使用 SAP (A)SCS 執行個體的 **Windows 容錯移轉叢集**和**共用磁碟**執行 SAP NetWeaver HA 安裝][sap-high-availability-installation-wsfc-shared-disk]
