...
| 已耗用           | 當日已耗用的資源量。 對於計算，我們會針對 VM 在指定的時間內運作的每一分鐘計費 (精確度高達 6 個小數位數)。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |    “1”, “0.5”                                                                                                                                                                                                                                                                                                                                                    |
...
對於 Azure 資源庫中的部分映像，系統會在 [其他資訊] 欄位中填入映像類型。 這可讓使用者了解並追蹤他們在虛擬機器上部署的類型。 根據您已部署的映像填入此欄位的值如下所示：
  - BitRock 
  - Canonical 
  - FreeBSD 
  - Open Logic 
...
ExtraSmall - ExtraLarge 是標準\_A0 – 標準\_A4 的舊版名稱。 在傳統 VM 使用情況記錄中，如果您已經部署這些大小，則可能會看到這個慣例的使用。
### <a name="what-is-the-difference-between-meter-region-and-resource-location"></a>計量區域與資源位置之間的差異為何？
計量區域與計量相關聯。 對於在所有區域使用一個價格的某些 Azure 服務，[計量區域] 欄位可能是空白。 不過，因為 VM 對於每個區域有專用的虛擬機器價格，因此會填入此欄位。 同樣地，虛擬機器的資源位置是 VM 部署所在的位置。 這兩個欄位中的 Azure 區域都相同，但是它們對於區域名稱可能會有不同的字串慣例。
...
VMName 只為針對擴展集中的 VM，填入 [其他資訊] 欄位中。 InstanceID 欄位包含非擴展集 VM 的 VM 名稱。
...
### <a name="how-do-i-know-if-i-am-charged-for-premium-software"></a>如何知道我是否支付進階軟體的費用？
探索最適合您需求的 VM 映像時，請務必簽出 [Azure Marketplace](https://azuremarketplace.microsoft.com/marketplace/apps/category/compute)。 該映像所使用的是軟體方案費率。 如果您看到費率為「免費」，則沒有額外的軟體成本。 
...
如果您透過傳統部署模型進行部署，則無法使用 InstanceID 字串。
...
在傳統模型中，資源的計費是在雲端服務層級彙總的。 如果雲端服務中有多個 VM 使用相同的計費計量，就會將您的使用情況彙總在一起。 透過 Resource Manager 部署的 VM 會在 VM 層級計費，因此此彙總將不適用。
