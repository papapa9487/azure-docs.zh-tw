...
[Azure 的虛擬網路 (VNet)](virtual-networks-overview.md) 是您在 Azure 所擁有的私人網路空間，可讓您將 Azure 資源安全地彼此連線。

虛擬網路對等互連可讓您完美地連線虛擬網路。 經過對等互連後，所有虛擬網路就可以作為一個整體來進行連線。 對等互連之虛擬網路中的虛擬機器彼此可以直接通訊。
在對等互連之虛擬網路中的虛擬機器之間的流量，會透過 Microsoft 骨幹基礎結構路由傳送，其原理就像在相同虛擬網路中的虛擬機器之間，流量只會透過「私人」IP 位址來路由傳送。

>[!IMPORTANT]
> 您可以將不同 Azure 區域中的虛擬網路對等互連。 此功能目前為預覽狀態。 您可以[註冊訂用帳戶以供預覽](virtual-network-create-peering.md)。 在相同區域中的對等互連虛擬網路通常是可用的。
>

使用虛擬網路對等互連的優點包括︰

* 通過虛擬網路對等互連的流量完全是私人的。 這些流量會通過 Microsoft 骨幹網路，完全不會涉及公用網際網路或閘道。
* 不同虛擬網路的資源之間具有低延遲、高頻寬連線。
* 經過對等互連後，就能在某個虛擬網路中使用來自另一個虛擬網路的資源。
* 虛擬網路對等互連可協助您跨 Azure 訂用帳戶、部署模型和跨 Azure 區域 (預覽) 傳輸資料。
* 能夠將透過 Azure Resource Manager 所建立的虛擬網路對等互連，或將透過 Resource Manager 所建立的虛擬網路與透過傳統部署模型所建立的虛擬網路對等互連。 若要深入了解兩個 Azure 部署模型之間的差異，請閱讀[了解 Azure 部署模型](../azure-resource-manager/resource-manager-deployment-model.md?toc=%2fazure%2fvirtual-network%2ftoc.json)一文。
...
虛擬網路中的虛擬機器可存取相同區域之對等互連虛擬網路中的內部負載平衡器。 在預覽版中，對於內部負載平衡器的支援不會延伸到全域對等互連的虛擬網路。 正式運作版的全域虛擬網路對等互連會支援內部負載平衡器。
...
您也可以有效地建立中樞和輪輻類型的環境，讓中樞得以裝載網路虛擬應用裝置等基礎結構元件。 所有輪輻虛擬網路可以接著與中樞虛擬網路對等互連。 流量可以通過在中樞虛擬網路中執行的網路虛擬設備。 簡單來說，虛擬網路 對等互連可讓使用者定義路由上的下一個躍點 IP 位址成為對等互連虛擬網路中虛擬機器的 IP 位址。 若要深入了解使用者定義的路由，請閱讀[使用者定義的路由概觀](virtual-networks-udr-overview.md)一文。 了解如何建立[中樞和輪輻網路拓撲](/azure/architecture/reference-architectures/hybrid-networking/hub-spoke?toc=%2fazure%2fvirtual-network%2ftoc.json#virtual network-peering)
...
例如，如果您要將名為 myvirtual networkA 和 myvirtual networkB 的虛擬網路對等互連，您的帳戶就必須獲派各個虛擬網路的下列最基本角色或權限：

|虛擬網路|部署模型|角色|權限|
|---|---|---|---|
|myvirtual networkA|Resource Manager|[網路參與者](../active-directory/role-based-access-built-in-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json#network-contributor)|Microsoft.Network/virtualNetworks/virtualNetworkPeerings/write|
...
|myvirtual networkB|Resource Manager|[網路參與者](../active-directory/role-based-access-built-in-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json#network-contributor)|Microsoft.Network/virtualNetworks/peer|
...
單一虛擬網路允許的對等互連數目有其限制。 對等互連的預設數目是 50。 您可以增加對等互連的數目。 如需詳細資訊，請檢閱 [Azure 網路限制](../azure-subscription-service-limits.md#networking-limits)。
