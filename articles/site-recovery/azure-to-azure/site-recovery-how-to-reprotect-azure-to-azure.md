...
---
# <a name="reprotect-azure-vms-back-to-the-primary-region"></a>重新保護 Azure VM 返回主要區域



>[!NOTE]
>
> Azure 虛擬機器 (VM) 的 Site Recovery 複寫目前為預覽狀態。


當您將 VM 從一個 Azure 區域[容錯移轉](../site-recovery-failover.md)到另一個 Azure 區域時，容錯移轉 VM 會處於未受保護狀態。 如果您需要使它們回復到主要區域，就必須先開始複寫 VM，然後再次進行容錯移轉。 朝任一個方向進行容錯移轉沒有任何差異。 同樣地，您啟用 VM 複寫之後，容錯移轉後或容錯回復後重新保護並沒有任何差異。

為說明重新保護程序，作為範例，我們假設受保護 VM 的主要站台是東亞地區，而復原站台為東南亞。 在容錯移轉期間，您要將 VM 容錯移轉到東南亞區域。 在容錯回復前，您必須將 VM 從東南亞複寫回到東亞。 本文說明使用重新保護所需的步驟。

> [!WARNING]
> 如果您已完成移轉，並將虛擬機器移至另一個資源群組 (或已刪除 Azure 虛擬機器)，就容錯回復。

完成重新保護且 VM 複寫之後，您可以在 VM 上起始容錯移轉，以將它們回復到東亞地區。

## <a name="prerequisites"></a>必要條件
1. VM 應該已認可。
2. 目標網站 (東亞) 應該可供使用，而且應該能夠存取/建立該區域的新資源。

## <a name="reprotect"></a>重新保護

請遵循下列步驟，使用預設設定重新保護 VM。

1. 在 [保存庫] > [已複寫的項目] 中，以滑鼠右鍵按一下已容錯移轉的 VM，然後選取 [重新保護]。 您也可以按一下機器，並從命令按鈕中選取**重新保護**。

  ![重新保護](./media/site-recovery-how-to-reprotect-azure-to-azure/reprotect.png)

2. 確認已選取複寫方向 [東南亞至東亞]。

  ![重新保護](./media/site-recovery-how-to-reprotect-azure-to-azure/reprotectblade.png)

3. 檢閱**資源群組、網路、儲存體和可用性設定組**資訊，並按一下 [確定]。 如果標記任何資源 (新的)，就會在重新保護期間加以建立。

這會觸發重新保護作業，在目標站台植入最新資料，一旦完成之後，請在容錯回復到東南亞之前複寫差異。

### <a name="reprotect-customization"></a>重新保護自訂
如果您需要在重新保護時選擇擷取儲存體帳戶或網路，可以使用重新保護頁面上提供的自訂選項。
...
|目標虛擬網路     | 重新保護期間無法變更網路。 若要變更網路，請重新進行網路對應。         |
...
3. 如果目標 AV 設定組不存在，而且預設值偵測到需要建立新的 AV 設定組，就會在重新保護作業中建立。 如果您已自訂重新保護，就會使用選取的 AV 設定組。


以下是將觸發重新保護作業時發生的步驟列出的清單。 這是以目標端虛擬機器存在的情況為準。
