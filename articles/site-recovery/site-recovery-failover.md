...
使用下表了解 Azure Site Recovery 針對不同的容錯移轉情節所提供的容錯移轉選項。
...
使用 Site Recovery 保護的虛擬機器/實體伺服器也支援**計劃性容錯移轉**。 這是完全不會遺失資料的容錯移轉選項。 觸發計劃性容錯移轉時，首先，來源虛擬機器會關機，尚未同步處理的資料會同步處理，然後觸發容錯移轉。
