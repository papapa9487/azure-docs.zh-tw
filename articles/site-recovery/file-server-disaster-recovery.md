...
**站對站連線能力**：必須在內部部署網站與 Azure 網路之間建立直接連線，以允許伺服器之間的通訊。  只要採用安全的站對站 VPN 連線，連線到將用作災害復原網站的 Microsoft Azure 虛擬網路，就可以確保能夠達成。  
...
6.  您的內部部署環境中萬一發生災害，請使用復原方案執行容錯移轉並新增指令碼，以[掛接 Azure 檔案共用](https://docs.microsoft.com/azure/storage/files/storage-how-to-use-files-windows)並存取虛擬機器中的共用。
...
下列步驟詳細說明 VMware VM 的複寫，如需複寫 Hyper-V VM 的步驟，請參閱這裡。
...
6.  您的內部部署環境中萬一發生災害，請使用復原方案執行容錯移轉並新增指令碼，以掛接 Azure 檔案共用並存取虛擬機器中的共用。
...
如需有關執行測試容錯移轉的詳細資訊，請參閱[這裡](site-recovery-failover.md)。
