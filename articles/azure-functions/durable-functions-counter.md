...
在範例所包含的 HTTP 觸發函式中，您可以使用下列 HTTP POST 要求來啟動協調流程。 為了讓 `counterState` 從零開始 (`int` 的預設值)，這個要求中沒有內容。
...
現在，您已充分了解 Durable Functions 的一些進階功能，特別是 `WaitForExternalEvent` 和 `ContinueAsNew`。 這些工具可讓您撰寫各種形式的「具狀態單次個體」，例如計數器和彙總工具。
