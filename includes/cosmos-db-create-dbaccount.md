...
3. 在 [新增帳戶] 頁面中，輸入新的 Azure Cosmos DB 帳戶的設定。 
    ...
    API|SQL|API 會決定要建立的帳戶類型。 Azure Cosmos DB 會提供五個 API，以符合應用程式的需求︰SQL (文件資料庫)、Gremlin (圖形資料庫)、MongoDB (文件資料庫)、Azure 資料表及 Cassandra，目前各自需要個別的帳戶。 <br><br>選取 [SQL]，因為在本快速入門中，您會使用 SQL 語法建立可查詢的文件資料庫且可使用 DocumentDB API 存取。<br><br>[深入了解 DocumentDB API](../articles/cosmos-db/documentdb-introduction.md)|
