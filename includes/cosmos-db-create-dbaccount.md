...
3. 在 [新增帳戶] 頁面中，輸入新的 Azure Cosmos DB 帳戶的設定。 
    ...
    API|SQL (DocumentDB)|API 會決定要建立的帳戶類型。 Azure Cosmos DB 會提供四個 API，以符合應用程式的需求︰Gremlin (圖形)、MongoDB、SQL (DocumentDB) 和資料表 (索引鍵-值)，目前各自需要個別的帳戶。 <br><br>選取 [SQL (DocumentDB)]**x**，因為在本快速入門中，您會使用 SQL 語法建立可查詢的文件資料庫。<br><br>[深入了解 DocumentDB API](../articles/cosmos-db/documentdb-introduction.md)|
