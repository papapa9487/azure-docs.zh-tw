...
IoT 中樞通訊模組會開啟從閘道到 IoT 中樞的單一 HTTP 連線。 HTTP 是 IoT 中樞所了解的三種通訊協定中的其中一種。 此模組可讓您透過一個連線進行所有裝置的多工連線，而不必為每一個裝置開啟連線。 這個方法可讓單一閘道連線許多裝置。 

在範例中，此模組會：

1. 取得具有 IoT 中樞裝置金鑰屬性的訊息，此屬性是由前一個模組指派。 
1. 使用 HTTP 通訊協定，將訊息內容傳送到 IoT 中樞。 
