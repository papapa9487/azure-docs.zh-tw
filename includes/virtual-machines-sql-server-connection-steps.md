...
9. 在 名稱 對話方塊中輸入此規則的名稱和描述，然後按一下完成。
...
3. 在 SQL Server Management Studio 物件總管中，以滑鼠右鍵按一下 SQL Server 執行個體的名稱 (虛擬機器名稱)，然後按一下屬性 。
   
    ![伺服器屬性](./media/virtual-machines-sql-server-connection-steps/20Server-Properties.png)
4. 在 安全性 頁面的 伺服器驗證 下方，選取 SQL Server 及 Windows 驗證模式，然後按一下確定。
