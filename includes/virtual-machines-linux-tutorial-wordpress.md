...
此 WordPress 設定用於概念證明。 如需生產安裝的詳細資訊和設定，請參閱 [WordPress 文件](https://codex.wordpress.org/Main_Page)。 
...
將 WordPress 設定為使用 MySQL 和 PHP。 執行下列命令來開啟您選擇的文字編輯器，並建立 `/etc/wordpress/config-localhost.php` 檔案：

```bash
sudo sensible-editor /etc/wordpress/config-localhost.php
```
將下列幾行複製到檔案，以您的資料庫密碼替代 *yourPassword* (其他值維持不變)。 然後儲存檔案。
...
在工作目錄中，建立文字檔 `wordpress.sql` 以設定 WordPress 資料庫： 

```bash
sudo sensible-editor wordpress.sql
```

新增下列命令，以您的資料庫密碼替代 *yourPassword* (其他值維持不變)。 然後儲存檔案。
